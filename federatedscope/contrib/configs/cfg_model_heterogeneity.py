...
def extend_model_heterogeneous_cfg(cfg):
    ...
    cfg.save_history_result_per_client = False  # 训练结束后是否保存每个客户端每一轮的评估指标
    cfg.save_history_result_avg = False  # 训练结束后是否保存所有客户端评估指标的均值
    # ------------------------------------------------------------------------------------------------------------------------------
    '''
    ...
    cfg.change_hidden_when_warpFC = False
    # options for the analytical experiments of FedPPN
    cfg.fedproto.only_CE_loss = False
    cfg.fedproto.use_similarity_for_inference = False
    ...
    cfg.fedapen.epoch_for_learn_weight = 1  # local training epoch for learning personalization weight

    # ------------------------------------------------------------------------ #
    # FedPPN
    # ------------------------------------------------------------------------ #
    cfg.FedPPN = CN()
    cfg.FedPPN.use_knn = False
    cfg.FedPPN.temp_idx = 1
    cfg.FedPPN.LP_layer = 1
    cfg.FedPPN.LP_alpha = 0.5
    cfg.FedPPN.temperature = 10
    cfg.FedPPN.proto_agg_type = 'train_loss'

    # ------------------------------------------------------------------------ #
    # FedPPN_Plus
    # ------------------------------------------------------------------------ #
    cfg.FedPPN_Plus = CN()
    cfg.FedPPN_Plus.learning_for_adaptability = False
    cfg.FedPPN_Plus.epoch_for_learn_weight = 1
    cfg.FedPPN_Plus.adaptive_weight_lr = 1e-3
    cfg.FedPPN_Plus.show_learned_weight_curve = False
    cfg.FedPPN_Plus.PPN_mode = 'PPN'
    cfg.FedPPN_Plus.use_knn_graph= False
    # ------------------------------------------------------------------------ #
...
def assert_mhfl_cfg(cfg):
    ...
    elif cfg.model.warpFC:
        cfg.model.out_channels = cfg.model.feature_dim
        if cfg.change_hidden_when_warpFC:
            cfg.model.hidden = cfg.model.feature_dim
        logger.info(
    ...
    if cfg.FedPPN_Plus.show_learned_weight_curve and not cfg.FedPPN_Plus.learning_for_adaptability:
        raise ValueError(
            f"cfg.FedPPN_Plus.show_learned_weight_curve is True,"
            f"but cfg.FedPPN_Plus.learning_for_adaptabilit is False."
            f"Please specify cfg.FedPPN_Plus.learning_for_adaptabilit as True."
        )
