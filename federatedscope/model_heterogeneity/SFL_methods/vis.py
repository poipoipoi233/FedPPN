...
def t_she_vis(node_emb, emb_labels, glob_protos, local_agg_protos_dict, state=0, client_id=None,
    ...
    if show_global_proto:
        if trainable_global_protos:
            ...
            label_temp = torch.Tensor(group_index).long().to('cuda:0')
        elif isinstance(glob_protos, dict):
            glob_proto_tensor = torch.stack(list(glob_protos.values()))
            label_temp = torch.Tensor(list(glob_protos.keys())).long().to('cuda:0')

        embedding_tensor = torch.cat(
            [embedding_tensor, glob_proto_tensor.to('cuda:0')])  # global prototype 和 embedding_tensor 做拼接
        emb_labels = torch.cat([emb_labels, label_temp])  # 将global prototype的标签添加至emb_labels的末尾
        ...
        if multiple_client:
            for client_id, proto_dict in local_agg_protos_dict.items():
                for proto_label, proto in proto_dict.items():
                    ...
                    emb_labels = torch.cat([emb_labels, torch.tensor(proto_label).unsqueeze(0).to('cuda:0')])
                    node2client.append(client_num)
        else:
            for local_proto_label, proto in local_agg_protos_dict.items():
                if isinstance(proto, list):
                    # 每个client的每个class有多个本地原型（质心）
                    temp = torch.cat(proto, dim=0).to('cuda:0')
                    temp_label = torch.full((len(proto),), local_proto_label).long().to('cuda:0')
                    ...
                    embedding_tensor = torch.cat([embedding_tensor, proto.unsqueeze(0)])
                    tmp_label = torch.tensor(local_proto_label).to('cuda:0')
    ...
    for i in range(num_labels):
        ...
        axs[0].scatter(embeddings_tsne[mask_emb, 0], embeddings_tsne[mask_emb, 1], color=colors(i),
                       label=str(unique_labels[i]),
                       s=10)
        axs[0].scatter(embeddings_tsne[mask_glob_proto, 0], embeddings_tsne[mask_glob_proto, 1], color=colors(i),
                       # label='glob_' + str(unique_labels[i]),
                       s=70, marker='^', edgecolors='black', zorder=3)
        axs[0].set_title(f'Rounds: {state}')
        if show_local_proto:
            mask4 = np.zeros(len(labels), dtype=bool)
            mask4[node_emb_num + global_proto_num:] = True
            mask_local_proto = mask1 & mask4
            axs[0].scatter(embeddings_tsne[mask_local_proto, 0], embeddings_tsne[mask_local_proto, 1],
                           color=colors(i),
    ...
    plt.xticks([])
    plt.yticks([])
    fig.savefig(f"scatter_plot_round_{state}.png", dpi=300)
    plt.show()
...
def load_emb_files(file_path, client_id):
    ...
    return node_emb_dict, node_labels, glob_protos_dict, local_agg_protos_dict, client_raw_data
...
print(os.getcwd())
filepath = 'D:\\ZHL_WORK\\Architecture_heterogeneous_FL\\FS3\\federatedscope\\model_heterogeneity\\embedding'
