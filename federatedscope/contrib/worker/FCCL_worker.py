...
class FCCLServer(Server):
    ...
    def trigger_for_start(self):
        ...
        if self.check_client_join_in():
            ...
            if self.task == 'cv':
                ...
                self.calculate_logits_output_for_node_classification()
            # TODO:lr没改
    ...
    def calculate_logits_output_for_node_classification(self):
        device = self._cfg.device
        lr = self._cfg.MHFL.public_train.optimizer.lr
        ...
        linear_output_list = dict()
        linear_output_target_list = dict()
        data = self.train_dataset[0].to(device)
        for model_idx, model in self.client_models.items():
            model.to(device)  #
            model.train()
            linear_output = model(data)
            linear_output_target_list[model_idx] = linear_output.clone().detach()
            linear_output_list[model_idx] = linear_output
        ...
        for model_idx, model in self.client_models.items():
            ...
            print('Communcation: ' + str(self.state) + ' Net: ' + str(model_idx) + 'Col: ' + str(col_loss.item()))
            col_loss.backward()
            optimizer.step()
