...
def simple_TSHE(embeddings, labels, client_id=0, round=0):
    ...
    for i in range(num_labels):
        ...
        axs[0].scatter(embeddings_tsne[label_mask, 0], embeddings_tsne[label_mask, 1], color=colors(i),
                       label=str(unique_labels[i]),
                       s=10)
        axs[0].set_title(f'Client #{client_id} rounds {round} class')
        axs[0].legend()
    # plt.legend()
    plt.show()
