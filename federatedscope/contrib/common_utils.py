...
def label_statistics_for_cv_dataset(data, modified_cfg, scaling):
    ...
    fig, axs = fig, axs = plt.subplots(1, 2, figsize=(10, 4), sharex=False, sharey=False)
...
def result_to_csv(result, init_cfg, best_round, runner, client_cfg_file):
    ...
    if 'fedppn' in method:
        out_dict['LP_layer'] = init_cfg.FedPPN.LP_layer
        out_dict['LP_alpha'] = init_cfg.FedPPN.LP_alpha
    if 'fedppn_plus' in method:
        out_dict['epoch_for_learn_weight'] = init_cfg.FedPPN_Plus.epoch_for_learn_weight
        out_dict['adaptive_weight_lr'] = init_cfg.FedPPN_Plus.adaptive_weight_lr
    ...
    if runner.cfg.save_history_result_avg:
        history_result_avg = runner.server.history_results['Results_avg']
        client_folder = f"{runner.cfg.result_floder}/{runner.cfg.exp_name}"
        if not os.path.exists(client_folder):
            os.makedirs(client_folder)
        with open(f"{client_folder}/avg_history_result.json", 'w') as f:
            json.dump(history_result_avg, f, indent=4)
        ...
        for key in history_result.keys():
            if ('test' in key or 'train' in key) and 'acc' in key:
                ...
                if nrows==1:
                    ...
                    axs[idx//ncols][idx%ncols].plot(x, y, label=key)
                    axs[idx//ncols][idx%ncols].xlabel = 'epoch'
                    axs[idx//ncols][idx%ncols].ylabel = 'accuracy'
                    axs[idx//ncols][idx%ncols].set_title(f'Client {client_id}:{model_type}')
                    axs[idx//ncols][idx%ncols].legend()
    plt.show()


def plot_learned_weight_curve(runner):
    cfg = runner.cfg
    client_num = cfg.federate.client_num
    fig, axs = plt.subplots(1, client_num, figsize=(12, 4), sharex=True, sharey=True)
    for client_id in range(1, client_num + 1):
        y = list(runner.client[client_id].client_learnded_weight.values())
        x = np.arange(1, len(y) + 1)
        idx = client_id - 1
        axs[idx].plot(x, y)
        axs[idx].xlabel = 'epoch'
        axs[idx].ylabel = 'learned_weight'
        axs[idx].set_title(f'Client {client_id}')
    plt.show()
    ...
    for client_id in range(1, client_num + 1):
        ...
        for key in history_result.keys():
            ...
            if value != 0.0 and 'acc' in key:
                best_round = history_result[key].index(value)
                print(f"best_{key}: {value}, best_round:{best_round} ")
            elif value == 0.0:
                print(f'can not find best_{key} in history_result')

        if runner.cfg.save_history_result_per_client:
            client_folder = f"{runner.cfg.result_floder}/{runner.cfg.exp_name}"
            if not os.path.exists(client_folder):
                os.makedirs(client_folder)
            with open(f"{client_folder}/client_{client_id}_history_result.json", 'w') as f:
                json.dump(history_result, f, indent=4)
    if runner.cfg.plot_acc_curve:
        plot_acc_curve(runner)
    if runner.cfg.FedPPN_Plus.show_learned_weight_curve:
        plot_learned_weight_curve(runner)
